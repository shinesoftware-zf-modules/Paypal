...
class Link extends AbstractHelper implements ServiceLocatorAwareInterface {
	...
	public function __invoke($receiver, $amount=0, $title="", $content="", $rawdata=false) {
	    $links = array();
	    $i = 0;
	    
	    if(empty($receiver)){ // User has not set the paypal account
	        return null;
	    }
	    
	    $serviceLocator = $this->getServiceLocator()->getServiceLocator();
	    $paypal = $serviceLocator->get('PaypalService');
	    ...
	    $mode = $settings->getValueByParameter('paypal', 'mode');
	    
	    
	    if($mode == "sandbox"){
	        $link = 'https://www.sandbox.paypal.com/cgi-bin/webscr?cmd=_ap-payment&paykey=PAYKEY';
	    }else{
	        $link = 'https://www.paypal.com/cgi-bin/webscr?cmd=_ap-payment&paykey=PAYKEY';
	    }
	     ...
	     if(!empty($matches['entire_string'])){
	        for ($j=0; $j < count($matches['entire_string']); $j++){
                try{
                    ...
                    $memo = $title . "\n" . $content;
                    $memo = $this->cleaner($memo);
                    $memo = $this->truncate($memo, "200");
                    
                    $key = $paypal->getPayKey($receiver, $amount, $memo, $user_currency);
                    $links[$i]['url'] = str_replace("PAYKEY", $key, $link);
                    $links[$i]['price'] = $amount;
                    $links[$i]['currency'] = $currency;
                    $links[$i]['title'] = $title;
	    ...
	    if(!empty($links)){
	        return $this->view->render('paypal/partial/paypal', array('links' => $links));
	    }
	    return null;
	}
